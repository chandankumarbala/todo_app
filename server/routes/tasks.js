...
module.exports = function tasksRouter(db) {
  ...
  router.patch('/:id', (req, res) => {
    const { text, deadline, completed, completed_at, position } = req.body
    ...
    db.prepare(
      'UPDATE tasks SET text=?, deadline=?, completed=?, completed_at=?, position=? WHERE id=?'
    ).run(updated.text, updated.deadline, updated.completed, updated.completed_at, updated.position, task.id)
